...
local playerTpBtn, s16 = createCell("player tp\n[CTRL+SPACE]", 16)
...
-- ESP BOX CHUẨN BOUNDING BOX (KHÔNG BỊ LỆCH MÀN HÌNH)
local espObjects = {}
...
local function createEspBox(player)
    ...
    nameLabel.Position = UDim2.new(0.5, -100, 0, -22)
...
local function executePlayerTP()
    local targetPart = currentTarget or getTargetInFOV()
    if targetPart then
        local c = localPlayer.Character
        local hrp = c and c:FindFirstChild("HumanoidRootPart")
        if hrp then
            local targetHrp = targetPart.Parent:FindFirstChild("HumanoidRootPart") or targetPart
            hrp.CFrame = targetHrp.CFrame * CFrame.new(0, 0, 3)
        end
    end
end

playerTpBtn.MouseButton1Click:Connect(executePlayerTP)
...
RunService.RenderStepped:Connect(function()
    ...
    -- TÍNH TOÁN VỊ TRÍ ESP BOX DÙNG BOUNDING BOX CHÍNH XÁC 100%
    if espActive then
        for player, obj in pairs(espObjects) do
            ...
            if char and hum and hum.Health > 0 then
                ...
                if onScreen then
                    -- Tính toán các điểm đỉnh 3D xung quanh BoundingBox để chiếu ra màn hình 2D
                    local extents = size / 2
                    local topPos = camera:WorldToViewportPoint((cframe * CFrame.new(0, extents.Y, 0)).Position)
                    local bottomPos = camera:WorldToViewportPoint((cframe * CFrame.new(0, -extents.Y, 0)).Position)
                    
                    local height = math.abs(topPos.Y - bottomPos.Y)
                    local width = height * 0.55
                    
                    obj.Box.Size = UDim2.new(0, width, 0, height)
                    obj.Box.Position = UDim2.new(0, screenPos.X - (width / 2), 0, topPos.Y)
                    
                    obj.Stroke.Color = rainbowColor
                    obj.Name.TextColor3 = rainbowColor
