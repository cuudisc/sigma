...
-- LOGIC TELEPORT RA SAU LƯNG TARGET (BEHIND TP)
playerTpBtn.MouseButton1Click:Connect(function()
    ...
    if targetPart then
        ...
        local hrp = c and c:FindFirstChild("HumanoidRootPart")
        if hrp then
            -- Lấy CFrame của RootPart mục tiêu (hoặc Character cha)
            local targetHrp = targetPart.Parent:FindFirstChild("HumanoidRootPart") or targetPart
            -- Dịch chuyển ra đằng sau lưng mục tiêu 3 studs và giữ hướng nhìn theo mục tiêu
